...
export const useBoardStore=create<Store>()(persist((set,get)=>({
  ...
  act:(actor,command)=>{
    ...
    const serialized=JSON.stringify({state:{...result.state,error:''},version:3});
    if(serialized.length>3_000_000){set({error:'브라우저 데모 저장 한도를 초과했습니다. 첨부 크기를 줄여 주세요.'});return false;}
    try {
      if(typeof window!=='undefined')window.localStorage.setItem('thesis-board-v3',serialized);
      set({...result.state,error:''});return true;
    } catch {set({error:'브라우저 저장 공간이 부족하거나 저장이 차단되었습니다. 파일을 줄이거나 저장 설정을 확인해 주세요.'});return false;}
  },
  reset:()=>{
    if(typeof window!=='undefined')Object.keys(localStorage).filter(key=>key.startsWith('thesis-form-v3-')).forEach(key=>localStorage.removeItem(key));
    set({...createSeed(),...selection,error:''});
  },
}),{name:'thesis-board-v3',version:3,partialize:({error:_,...state})=>state}));
