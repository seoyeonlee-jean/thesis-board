...
export type StepType = 'APPLICATION' | 'ADVISOR_REQUEST' | 'COURSE_ENROLLMENT' | 'PLAN_SUBMISSION' | 'MEETING' | 'CHECKPOINT' | 'PRESENTATION' | 'FINAL_SUBMISSION' | 'EXTERNAL_SUBMISSION';
...
export interface Stage {
  ...
  stepType?: StepType; attendanceRequired?: boolean; consequence?: string;
  approver?: 'ADVISOR' | 'NONE' | 'EXTERNAL'; submitTo?: 'IN_SERVICE' | 'DEPT_OFFICE' | 'EMAIL' | 'OTHER';
  requiresAdvisorSignature?: boolean; conditionalDeadlines?: {condition:string;deadline:string}[];
  constraints?: string[]; dateStatus?: 'CONFIRMED' | 'PLACEHOLDER'; isPlaceholder?: boolean;
  dateNote?: string; requiresVideo?: boolean; collectsReviewOutcome?: boolean;
}
export interface Department {
  ...
  courseNames?: string[]; maxStudentsPerAdvisor?: number;
}
...
export interface Draft { id: string; studentId: string; departmentId: string; stageId: string; professorId: string; title: string; summary: string; body: string; file?: Attachment; meetingWanted: boolean; savedAt: string; videoUrl?: string; duplicateAcknowledged?: boolean; }
export interface Application extends Omit<Draft, 'savedAt'> { status: ApplicationStatus; requestedAt: string; feedback?: Feedback; }
export interface Submission { id: string; studentId: string; departmentId: string; stageId: string; professorId: string; kind: 'plan' | 'final'; title: string; body: string; file?: Attachment; version: number; status: ReviewStatus | '접수됨'; submittedAt: string; feedback?: Feedback; videoUrl?: string; }
export interface Meeting { id: string; applicationId: string; studentId: string; professorId: string; departmentId: string; slots: string[]; selected?: string; status: '시간 제안됨' | '확정' | '완료'; location: string; stageId?: string; }
...
export interface ReviewOutcome {studentId:string;departmentId:string;stageId:string;condition:string;at:string;}
