...
export function Roadmap({state,student,department,preview=false}:{state:BoardState;student:Student;department:Department;preview?:boolean}){
 ...
 return <ol className="roadmap" aria-label={preview?'로드맵 미리보기':'전공 로드맵'}>{steps.map(step=>{
  ...
  return <li key={step.id} ref={step.id===current?.id?ref:undefined} id={preview?undefined:'step-'+step.id} className={'roadmap-step '+(status==='예정'?'opacity-65':'')} data-status={status}>
   ...
   <details className={'rounded-xl border p-4 '+(step.id===current?.id?'border-snu-300 bg-snu-50/30':'bg-white')} open={!preview&&step.id===current?.id}>
    <summary className="cursor-pointer"><b>{step.name}</b><span className="mt-2 flex flex-wrap gap-2"><StageDeadlines state={state} studentId={student.id} department={department} step={step}/><StageBadges step={step} assistant={preview}/><Badge className={status==='마감 지남'?'bg-red-50 text-red-700':status==='보완 필요'?'bg-orange-50 text-orange-800':colorFor(department)}>{status}</Badge>{changed&&<Badge className="bg-amber-50 text-amber-800">일정 변경</Badge>}{step.required&&<Badge>필수</Badge>}</span>{step.consequence&&<span className="mt-2 block text-sm text-red-800">{step.consequence}</span>}</summary>
    <div className="mt-3 space-y-2 text-sm text-slate-600"><p>{step.description}</p>{step.dateNote&&<p className="text-xs">일정 근거: {step.dateNote}</p>}{step.constraints?.map(c=><p key={c} className="text-amber-900">• {c}</p>)}{state.completions.filter(v=>v.studentId===student.id&&v.departmentId===department.id&&v.stageId===step.id).map(v=><p key={v.at}>완료 기록: {dateLabel(v.at)} (서비스 내 기록)</p>)}<p>제출물: {step.documents||'정보 확인 필요'}</p><p>제출처: {step.submissionMethod||'정보 확인 필요'}</p><p>유의사항: {step.notes||'없음'}</p><p>문의: {step.contact||'정보 확인 필요'}</p><FileView file={step.file}/>{state.notices.filter(n=>noticeMatches(n,student,department)&&n.stageId===step.id).map(n=><div className="rounded bg-amber-50 p-3" key={n.id}><b>{n.title}</b><p>{n.body}</p><FileView file={n.file}/></div>)}</div>
   </details>
  </li>;
 })}</ol>;
}
