...
export function Roadmap({state,student,department,preview=false}:{state:BoardState;student:Student;department:Department;preview?:boolean}){
 ...
 return <ol className="roadmap" aria-label={preview?'로드맵 미리보기':'전공 로드맵'}>{steps.map(step=>{
  ...
  return <li key={step.id} id={preview?undefined:'step-'+step.id} className={'roadmap-step '+(status==='예정'?'opacity-65':'')} data-status={status}>
   ...
   <details className={'rounded-xl border p-4 '+(step.id===current?.id?'border-snu-300 bg-snu-50/30':'bg-white')}>
