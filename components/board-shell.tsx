...
export function BoardShell({children}:{children:React.ReactNode}){
 ...
 return <><header className="border-b bg-white"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3"><Link href="/" className="font-extrabold text-snu">논문 보드</Link><nav aria-label="역할 전환" className="flex rounded-xl bg-slate-100 p-1">{links.map(([label,href])=><Link key={href} href={href} className={'rounded-lg px-3 py-2 text-sm font-bold '+(path===href?'bg-white text-snu shadow-sm':'text-slate-500')}>{label}</Link>)}</nav><div className="flex flex-wrap items-center gap-2">{ready&&<select aria-label="데모 사용자" value={id} onChange={e=>{state.choose(role,e.target.value);setOpen(false);}} className="max-w-40 rounded-lg border p-2 text-sm">{people.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>}<button className="btn-alt" aria-label="알림" aria-expanded={open} onClick={()=>setOpen(!open)}>🔔 알림 {ready?notifications.filter(n=>!n.read).length:0}</button><button className="btn-alt" onClick={()=>{if(confirm('현재 학과 자료 데모의 저장 내용을 초기화할까요?'))state.reset();}}>데모 초기화</button></div></div></header>
 ...
 <main className="mx-auto max-w-7xl px-4 py-7">{ready?<>{state.error&&<div role="alert" className="mb-4 flex items-center justify-between gap-3 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">{state.error}<button onClick={state.clearError}>닫기</button></div>}{children}</>:<p>저장된 데모를 불러오는 중…</p>}</main><footer className="px-4 pb-8 text-center text-xs text-slate-500">실제 자료 + 일부 가정 · 브라우저별 저장 (팀원 간 동기화 없음) · 실제 학사 처리 아님</footer></>;
}
