...
test('네 학과 자료·필참·출처와 외부 분반 표시',async({page},info)=>{
 ...
 await page.getByLabel('데모 사용자').selectOption('student-4');await expect(page.getByLabel('전공 로드맵').locator(':scope > li')).toHaveCount(7);await expect(page.getByLabel('다음 할 일')).toContainText('논문계획서 초안 제출');
...
test('산공 계획서부터 수정 후 합격 마감·외부 제출 기록까지',async({page})=>{
 ...
 await studentTab(page,'로드맵');await page.getByRole('button',{name:'제출 완료 체크'}).click();await expect(page.getByLabel('다음 할 일')).toContainText('모든 단계 완료');await office.locator('summary').click();await expect(office.getByText('완료 기록:',{exact:false})).toBeVisible();await page.reload();await expect(page.getByLabel('다음 할 일')).toContainText('모든 단계 완료');
});
