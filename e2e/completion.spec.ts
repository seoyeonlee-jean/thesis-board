...
test('전공 탭·요약 이동·색상·달력 필터',async({page})=>{
 ...
 await studentTab(page,'캘린더');const calendar=page.getByLabel('통합 캘린더');const a=await calendar.getByRole('button',{name:'지도교수 신청',exact:true}).evaluate(e=>getComputedStyle(e).backgroundColor);const b=await calendar.getByRole('button',{name:'지도교수 신청·승인',exact:true}).evaluate(e=>getComputedStyle(e).backgroundColor);expect(a).not.toBe(b);
