...
import {sendApplication,studentApplication,pdfFile,noOverflow,studentTab} from './helpers';
test('공지 대상·연결 단계·캘린더·읽음',async({page})=>{
 ...
 await page.getByRole('link',{name:'학생',exact:true}).click();await studentTab(page,'공지');await expect(page.locator('#notices').getByText('변경된 일정 안내')).toBeVisible();await studentTab(page,'로드맵');await expect(page.locator('#step-apply').getByText('변경된 일정 안내')).toBeVisible();await studentTab(page,'캘린더');await page.getByLabel('캘린더 월').fill('2026-10');await expect(page.getByLabel('월간 일정').getByRole('button',{name:'변경된 일정 안내',exact:true})).toBeVisible();
...
test('정원·모집 검증과 신청 수정 재제출',async({page})=>{
 ...
 await page.getByRole('button',{name:'지도 신청 제출'}).click();await page.getByRole('link',{name:'교수',exact:true}).click();await page.getByRole('tab',{name:'지도 신청',exact:true}).click();await page.getByRole('button',{name:'정원·모집 수정'}).click();await page.getByLabel('지도 정원',{exact:true}).fill('1');await expect(page.getByRole('button',{name:'정원 저장'})).toBeDisabled();await page.getByLabel('지도 정원',{exact:true}).fill('5');await page.getByRole('button',{name:'정원 저장'}).click();
 await page.getByLabel('처리 피드백').fill('연구 범위를 좁혀 주세요.');await page.getByRole('button',{name:'수정 요청',exact:true}).click();await page.getByRole('link',{name:'학생',exact:true}).click();await studentTab(page,'지도교수');await page.getByLabel('연구계획 본문').fill('좁힌 범위');await page.getByRole('button',{name:'재제출',exact:true}).click();await expect(page.getByText('제출됨',{exact:true})).toBeVisible();
});
test('절차 마지막 추가·삭제·수업형 게시',async({page})=>{
 await page.goto('/admin');await page.getByRole('button',{name:'단계 추가',exact:true}).click();await expect(page.getByLabel('절차 단계 편집').locator(':scope > li').last()).toContainText('새 단계');
...
test('375px 절차 편집기·교수 시간 그리드 레이아웃',async({page})=>{
 await page.setViewportSize({width:375,height:900});await page.goto('/admin');await noOverflow(page);await sendApplication(page);await page.getByRole('link',{name:'교수',exact:true}).click();await page.getByRole('tab',{name:'지도 신청',exact:true}).click();await page.getByRole('button',{name:'면담 요청',exact:true}).click();await noOverflow(page);
});
test('교수 면담 그리드 드래그로 연속 시간 선택',async({page})=>{
 await sendApplication(page);await page.getByRole('link',{name:'교수',exact:true}).click();await page.getByRole('tab',{name:'지도 신청',exact:true}).click();await page.getByRole('button',{name:'면담 요청',exact:true}).click();
...
test('알림이 다른 전공 탭의 상세로 이동',async({page})=>{
 await page.goto('/student');await page.getByRole('tab',{name:'중어중문학과',exact:true}).click();await studentTab(page,'연구계획·논문');await page.getByLabel('제출물 제목').fill('논문 개요');await page.getByLabel('제출물 본문').fill('고전 문학 개요');await page.getByRole('button',{name:'제출물 제출',exact:true}).click();
 ...
 await page.getByLabel('알림',{exact:true}).click();await page.getByLabel('알림 목록').getByRole('link',{name:/지도 신청 승인/}).click();await expect(page.getByRole('tab',{name:'중어중문학과',exact:true})).toHaveAttribute('aria-selected','true');await expect(page.getByLabel('다음 할 일')).toContainText('지도교수 면담·작성');
});
test('절차 기본 접힘·고정 순서·요약·마감 표시',async({page},info)=>{
 await page.goto('/admin');const items=page.getByLabel('절차 단계 편집').locator(':scope > li');
 await expect(items.locator('details[open]')).toHaveCount(0);await expect(page.getByLabel('로드맵 미리보기').locator('details[open]')).toHaveCount(0);
 await expect(page.getByLabel('드래그 핸들')).toHaveCount(0);await expect(page.getByRole('button',{name:/위로|아래로/})).toHaveCount(0);await expect(items.locator('[draggable]')).toHaveCount(0);
 await expect(items.first()).toContainText('2026. 10. 14. 18:00');await expect(page.getByLabel('로드맵 미리보기').locator(':scope > li').first()).toContainText('10월 14일 18:00까지');
 await items.first().getByText('단계 설정',{exact:true}).click();await expect(items.locator('details[open]')).toHaveCount(1);await items.first().getByLabel('마감일',{exact:true}).fill('2026-10-02T18:00');
 await expect(items.first()).toContainText('2026. 10. 02. 18:00');await expect(page.getByLabel('로드맵 미리보기').locator(':scope > li').first()).toContainText('10월 2일 18:00까지');
 await items.first().getByText('단계 설정',{exact:true}).click();await page.setViewportSize({width:375,height:900});await noOverflow(page);await page.screenshot({path:info.outputPath('collapsed-editor.png'),fullPage:true});
});
