...
for(const width of [1440,375])test('학생 영역 탭·SNU Blue·초안 보존 '+width+'px',async({page},info)=>{
 await page.setViewportSize({width,height:900});await page.goto('/student#roadmap');
 ...
 await page.getByLabel('캘린더 월').fill('2026-09');await page.getByLabel('통합 캘린더').getByRole('button',{name:'지도교수 신청·승인',exact:true}).click();
...
test('학생 업무 키보드 탐색과 해시 링크',async({page})=>{
 ...
 await expect(tabs.getByRole('tab',{name:'공지',exact:true})).toBeFocused();
 await page.keyboard.press('End');await expect(tabs.getByRole('tab',{name:'공지',exact:true})).toBeFocused();
 await page.keyboard.press('Home');await expect(tabs.getByRole('tab',{name:'캘린더',exact:true})).toBeFocused();
