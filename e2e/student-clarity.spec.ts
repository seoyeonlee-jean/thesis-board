...
for(const width of [1440,375])test('샤논 학생 화면 선택 축소·접힘·진행률·첫 화면 '+width,async({page},info)=>{
 await page.clock.setFixedTime(new Date('2026-09-22T09:00:00+09:00'));await page.setViewportSize({width,height:900});await page.goto('/student#roadmap');
 ...
 await expect(page.getByLabel('전공 로드맵').locator('details[open]')).toHaveCount(1);
 ...
 await expect(page.getByRole('tablist',{name:'학생 업무'}).getByRole('tab')).toHaveText(['캘린더','로드맵','지도교수','면담 일정','연구계획서·논문','공지']);
 await expect(page.locator('#step-apply').getByRole('button')).toHaveText('지도교수 찾아 신청하기');
 await expect(page.getByRole('main')).not.toContainText('예시 데이터');await expect(page.getByRole('main')).not.toContainText('예시 링크(가상)');
 const next=await page.getByLabel('현재 단계').boundingBox(),roadmap=await page.locator('#roadmap').boundingBox();expect(next!.height).toBeLessThan(200);expect(roadmap!.y).toBeLessThan(780);
 await page.getByRole('button',{name:'중어중문학과 전공 선택'}).click();await expect(page.getByRole('button',{name:'중어중문학과 전공 선택'})).toHaveAttribute('aria-pressed','true');await expect(page.getByLabel('현재 단계')).toContainText('논문제출신청·개요 제출');await expect(page.getByLabel('전공 로드맵').locator('details[open]')).toHaveCount(1);
 ...
 await page.goto('/student#roadmap');await expect(page.getByLabel('전공 로드맵').locator('details[open]')).toHaveCount(1);
});
