...
for(const width of [1440,375])test('샤논 학생 화면 선택 축소·접힘·진행률·첫 화면 '+width,async({page},info)=>{
 ...
 await page.getByRole('button',{name:'중어중문학과 전공 선택'}).click();await expect(page.getByRole('button',{name:'중어중문학과 전공 선택'})).toHaveAttribute('aria-pressed','true');await expect(page.getByLabel('다음 할 일')).toContainText('논문제출신청·개요 제출');await expect(page.getByLabel('전공 로드맵').locator('details[open]')).toHaveCount(1);
