...
export async function studentApplication(page:Page){
 await page.goto('/student');await studentTab(page,'지도교수');await page.getByRole('button',{name:'서진우 교수 선택'}).click();
...
export async function studentTab(page:Page,name:string){await page.getByRole('tablist',{name:'학생 업무'}).getByRole('tab',{name,exact:true}).click();}
